    ...
    if (err) {
      ...
      if (!accountId) {
        ...
        // If both exist
          ...
          if (
            addedIncludesCurrentAccountIn.approved ||
            currentIncludesAddedAccountIn.approved
          ) {
            // Sending a response
            return res.json({
              message: "Friends",
              code: "friends".toUpperCase(),
            });
          }
          // If the other account has requested to be friends with current account
            ...
            const approvalOfCurrentAccount =
              currentAccount.friends.approved.create({
                accountId: addedAccount._id,
              });
          ...
          // If the friend request is pending from the added account
            ...
            const requestObject = _.each(
              currentAccount.friends.requested,
              (account) => {
